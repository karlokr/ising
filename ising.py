...
class ising:
    ...
    def ising2D(self, T, N, J, init_grid=None, plot_flag=False):
        ...
        if init_grid == None:
            for k in range(t):
                ...
                if (k > 10*N*N) & (k % (10*N*N) == 0):
                    M_h = np.sum(
                        np.abs(Mlist_h[k-N*N*10:]))/np.size(Mlist_h[k-N*N*10:])/(N*N)
                    E_h = np.sum(Elist_h[k-N*N*10:]) / \
                        np.size(Elist_h[k-N*N*10:])/(N*N)
                    M_c = np.sum(
                        np.abs(Mlist_c[k-N*N*10:]))/np.size(Mlist_c[k-N*N*10:])/(N*N)
                    E_c = np.sum(Elist_c[k-N*N*10:]) / \
                        np.size(Elist_c[k-N*N*10:])/(N*N)
                    diff = np.sqrt(np.power(
                        np.abs(M_c - M_h)/np.abs(M_h), 2) + np.power(np.abs(E_c-E_h)/np.abs(E_h), 2))
                    if diff < 0.05:
                        grid = grid_h
                        break
                if int(k) == int(t-1):
        ...
        # re-initialize for the thermalized grid config
        Energy = self.__nn_energy2D(grid, J)
        ...
        chi = (np.sum(Mlist**2)/np.size(Mlist) -
               np.sum(Mlist)**2/np.size(Mlist)**2)/T/(N*N)
        Cv = (np.sum(Elist**2)/np.size(Elist)-np.sum(Elist)
              ** 2/np.size(Elist)**2)/(T**2)/(N*N)
        ...
        if plot_flag:
            if init_grid == None:
                ...
                plt.title("Evolution of MCMC for T=" +
                          str(T)+", N="+str(N)+", J="+str(J))
                plt.xlabel("Iteration")
                plt.ylabel("Energy / Magnetization")
                #plt.legend(loc='center right')
                plt.tight_layout()
                plt.savefig('Ising2D T=' + str(T)+", N=" +
                            str(N)+", J="+str(J)+'.png', dpi=600)
            else:
    ...
    def ising1D(self, T, N, J, init_grid=None, hot_start=True):
        N = int(N)
        J = float(J)
        T = float(T)
